...
- Syntax (name/value pairs only; no directives):
  - `<key>=<value>`
...
- `Set-Cookie: id=X6kAwpgW29M; Expires=Wed, 07 Feb 2024 16:35:00 GMT`
...
- Use the expires or max-age directive to make sure this is not a session cookie
...
- Yes, it's confusing that we overload the word session
  - It's a session cookie in that it tracks a user's session with our app
...
- The server sees that there is a cookie in this request and looks up the token in the database to retrieve this user's profile, including their username
...
- With multiple users:
  ...
  - The tokens are all secret so users cannot steal each other's identities
